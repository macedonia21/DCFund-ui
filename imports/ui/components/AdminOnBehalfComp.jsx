...
class AdminOnBehalfComp extends Component {
    constructor(props) {
        super(props);
    ...
    fetchUsers() {
        ...
        Meteor.call('admin.getUserProfile', true, (err, res) => {
            if (err) {
    ...
    sendCoin() {
        ...
        const walletKey = this.state.userPubKey;
        const walletOwner = this.state.userFullName;
    ...
    onChangeUser() {
        ...
        if (userIndex && userIndex >= 0 && this.state.users[userIndex]) {
            ...
            this.setState({userFullName: user.profile.fullName});
        } else {
            this.setState({userAddress: null});
            this.setState({userPubKey: null});
